...
from pspnet.util import *
from pspnet.loss import *
...
class pspnet(nn.Module):
    ...
    def __init__(self,  n_classes=21, block_config=[3, 4, 23, 3], input_size=(473,473), version=None):
        ...
        # Encoder
        self.convbnrelu1_1 = conv2DBatchNormRelu(in_channels=3, k_size=3, n_filters=64, padding=1, stride=2, bias=False) # 64
        self.convbnrelu1_2 = conv2DBatchNormRelu(in_channels=64, k_size=3, n_filters=64, padding=1, stride=1, bias=False) # 64, 64
        self.convbnrelu1_3 = conv2DBatchNormRelu(in_channels=64, k_size=3, n_filters=128, padding=1, stride=1, bias=False) # 64, 128
        # self.convbnrelu1_1 = conv2DBatchNormRelu(in_channels=3, k_size=3, n_filters=4, padding=1, stride=2, bias=False) # 64
        # self.convbnrelu1_2 = conv2DBatchNormRelu(in_channels=4, k_size=3, n_filters=4, padding=1, stride=1, bias=False) # 64, 64
        # self.convbnrelu1_3 = conv2DBatchNormRelu(in_channels=4, k_size=3, n_filters=8, padding=1, stride=1, bias=False) # 64, 128

        # Vanilla Residual Blocks
        self.res_block2 = residualBlockPSP(self.block_config[0], 128, 64, 256, 1, 1) # 128, 64, 256, 1, 1
        self.res_block3 = residualBlockPSP(self.block_config[1], 256, 128, 512, 2, 1) # 256, 128, 512, 2, 1
        # self.res_block2 = residualBlockPSP(self.block_config[0], 8, 4, 16, 1, 1) # 128, 64, 256, 1, 1
        # self.res_block3 = residualBlockPSP(self.block_config[1], 16, 8, 32, 2, 1) # 256, 128, 512, 2, 1
        
        # Dilated Residual Blocks
        self.res_block4 = residualBlockPSP(self.block_config[2], 512, 256, 1024, 1, 2) # 512, 256, 1024, 1, 2
        self.res_block5 = residualBlockPSP(self.block_config[3], 1024, 512, 2048, 1, 4) # 1024, 512, 2048, 1, 4
        # self.res_block4 = residualBlockPSP(self.block_config[2], 32, 16, 64, 1, 2) # 512, 256, 1024, 1, 2
        # self.res_block5 = residualBlockPSP(self.block_config[3], 64, 32, 128, 1, 4) # 1024, 512, 2048, 1, 4
        
        # Pyramid Pooling Module
        self.pyramid_pooling = pyramidPooling(2048, [6, 3, 2, 1]) # 2048, [6, 3, 2, 1]
        # self.pyramid_pooling = pyramidPooling(128, [6, 3, 2, 1]) # 2048, [6, 3, 2, 1]
       
        # Final conv layers
        self.cbr_final = conv2DBatchNormRelu(4096, 512, 3, 1, 1, False) # 4096, 512, 3, 1, 1, False
        # self.cbr_final = conv2DBatchNormRelu(256, 32, 3, 1, 1, False) # 4096, 512, 3, 1, 1, False
        self.dropout = nn.Dropout2d(p=0.1, inplace=False)
        self.classification = nn.Conv2d(512, self.n_classes, 1, 1, 0) # 512, self.n_classes, 1, 1, 0
        # self.classification = nn.Conv2d(32, self.n_classes, 1, 1, 0) # 512, self.n_classes, 1, 1, 0

        # Auxiliary layers for training
        self.convbnrelu4_aux = conv2DBatchNormRelu(in_channels=1024, k_size=3, n_filters=256, padding=1, stride=1, bias=False) # in_channels=1024, k_size=3, n_filters=256, padding=1, stride=1, bias=False
        self.aux_cls = nn.Conv2d(256, self.n_classes, 1, 1, 0) # 256, self.n_classes, 1, 1, 0
        # self.convbnrelu4_aux = conv2DBatchNormRelu(in_channels=64, k_size=3, n_filters=16, padding=1, stride=1, bias=False) # in_channels=1024, k_size=3, n_filters=256, padding=1, stride=1, bias=False
        # self.aux_cls = nn.Conv2d(16, self.n_classes, 1, 1, 0) # 256, self.n_classes, 1, 1, 0
